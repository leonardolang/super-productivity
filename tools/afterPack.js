...
async function afterPack(context) {
  ...
  const installWaylandIdleHelper = async () => {
    ...
    if (!helperStat) {
      if (isTruthyEnv(process.env.SP_EXCLUDE_WAYLAND_IDLE_HELPER)) {
        console.warn(
          '[afterPack] Skipping Wayland idle helper copy because SP_EXCLUDE_WAYLAND_IDLE_HELPER is set',
        );
        return;
      }

      throw new Error(
        `[afterPack] ${WAYLAND_IDLE_HELPER_SRC} not found. Linux packages must include the Wayland idle helper; install Rust/Cargo before packaging or set SP_EXCLUDE_WAYLAND_IDLE_HELPER=1 to intentionally omit ext-idle-notify support.`,
      );
    }
